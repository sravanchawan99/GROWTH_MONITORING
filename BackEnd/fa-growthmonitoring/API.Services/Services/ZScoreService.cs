    ...
    public class ZScoreService
    {
        ...
        public async Task<ZscoreResult> ProvideZscore(Beneficiary child)
        {
            try
            {
                if (child.height > 120)
                {
                    _logger.LogWarning("Height {Height} exceeds the supported limit of 120 cm.", child.height);

                    return new ZscoreResult
                    {
                        Wasting = "This module works only for children with height below 120 cm.",
                        Underweight = "N/A",
                        Stunting = "N/A",
                        NWL = 0,
                        NWA = 0,
                ...
                // Weight for Length
                _logger.LogInformation("Calculating Wasting.....");
                child.type = "WLZ";
                ...
                if (lms != null)
                {
                    _logger.LogInformation(
                        "LMS Values for Weight for Length as per WHO are L:{L}, M:{M}, S:{S}",
                        lms.L, lms.M, lms.S);
                }
                ...
                _logger.LogInformation("Wasting: {Wasting}", wasting);

                // Weight for Age
                _logger.LogInformation("Calculating Underweight.....");
                child.type = "WAZ";
                ...
                {
                    _logger.LogInformation(
                        "LMS Values for Weight for Age as per WHO are L:{L}, M:{M}, S:{S}",
                        lms1.L, lms1.M, lms1.S);
                }
                ...
                _logger.LogInformation("Under Weight: {Underweight}", underweight);

                // Height for Age
                _logger.LogInformation("Calculating Stunting.....");
                child.type = "LAZ";
                ...
                {
                    _logger.LogInformation(
                        "LMS Values for Height for Age as per WHO are L:{L}, M:{M}, S:{S}",
                        lms2.L, lms2.M, lms2.S);
                }
                ...
                _logger.LogInformation("Stunting: {Stunting}", stunting);
            ...
            {
                _logger.LogError(ex, "Error while calling LMS API.");
                throw;
            }
            catch (JsonException ex)
            {
                _logger.LogError(ex, "Error while deserializing LMS response.");
                throw;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Unexpected error occurred while calculating Z-Score.");
                throw;
            }
        }
        ...
        private string Categorize(double zscore)
        {
            string res = "";
            if (zscore >= -2)
                res = "Normal";
